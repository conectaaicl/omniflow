...
export default function BroadcastsPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/broadcasts', {
        name: form.name, message: form.message, channel: form.channel,
      ...
      setForm({ name: '', message: '', channel: 'whatsapp', segment_tags: '', segment_min_score: '', segment_source: '' })
      setShowForm(false)
  ...
  const handleSend = async (b: any) => {
    if (!window.confirm(`¿Enviar "${b.name}" a ${b.recipient_count} contactos?`)) return
    setSending(b.id)
    try {
    ...
    <div className="min-h-screen bg-[#F8F7FF]">
      ...
      <div className="bg-white border-b border-gray-100 px-8 py-5 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">Envía campañas segmentadas a tus contactos</p>
        </div>
      ...
      <div className="p-8 space-y-6">
        ...
        {showForm && (
          <form onSubmit={handleCreate} className="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
            ...
            <div className="border-t border-gray-100 pt-4">
              <p className="text-xs font-semibold text-gray-500 mb-3 uppercase tracking-wide">Segmentacion (opcional)</p>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <div>
                  <label className="block text-[10px] text-gray-400 mb-1">Score minimo</label>
        ...
        {broadcasts.length > 0 && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              { label: 'Enviadas', value: broadcasts.filter(b => b.status === 'sent').length },
        ...
        {loading ? (
          ...
          <div className="text-center py-16 text-gray-400">
            ...
            <p className="text-sm mt-1">Crea tu primera campaña masiva</p>
          </div>
        ) : (
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        ...
                        <span className="text-[10px] text-gray-400 capitalize">{b.channel}</span>
                      </div>
                    ...
                    <div className="flex-shrink-0 text-right text-xs text-gray-500">
                      <div>{b.sent_count || 0} / {b.recipient_count || 0} enviados</div>
